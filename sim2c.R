##########################################################################
####  Optimal Reinsurance Multivariate: Simulation Study 2 (Case 3)   ####
##########################################################################
...
## -------------------- 3.  functions ---------------
## 3.1  random losses  X  ~  t-copula + Uniform
...
print(summary_df)
mean(VaR_qs_vec^2-VaR_sl_vec^2,na.rm=T)/n
c(mean(VaR_qs_vec^2-VaR_sl_vec^2,na.rm=T)-1.96*sd(VaR_qs_vec^2-VaR_sl_vec^2,na.rm=T),
  mean(VaR_qs_vec^2-VaR_sl_vec^2,na.rm=T)+1.96*sd(VaR_qs_vec^2-VaR_sl_vec^2,na.rm=T))/n
## actual vs estimated 
print(apply(cbind(c_mat,d_mat,VaR_qs_vec,VaR_sl_vec),2,function(x)sd(x,na.rm=T)))
...
###############################################################################
save(c_mat, d_mat, VaR_qs_vec, VaR_sl_vec, boot_sd_mat, file="sim2c_3.Rda")
